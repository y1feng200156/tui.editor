...
class WwPasteContentHelper {
  ...
  _wrapOrphanNodeWithDiv($container) {
    ...
    util.forEachArray(array, node => {
      ...
      const isInlineNode = /^(SPAN|A|CODE|EM|I|STRONG|B|S|ABBR|ACRONYM|CITE|DFN|KBD|SAMP|VAR|BDO|Q|SUB|SUP|U)$/ig.test(node.tagName);
  ...
  _unwrapNestedBlocks($container, blockTags) {
    const $leafElements = $container.find(':not(:has(*))').not('b,s,i,em,code,span,u');
